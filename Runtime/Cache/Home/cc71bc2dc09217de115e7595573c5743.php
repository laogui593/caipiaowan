<?php if (!defined('THINK_PATH')) exit();?><!DOCTYPE html>
<html>
	<head>
		<meta charset="utf-8" />
		
		<title><?php echo C('sitename');?></title>

		<meta name="renderer" content="webkit">
		<meta name="format-detection" content="telephone=no,email=no"/>
		<meta name="viewport" content="width=device-width, initial-scale=1.0, maximum-scale=1.0, minimum-scale=1.0, user-scalable=0">
		<meta name="telephone=no" content="format-detection">
		<meta name="apple-mobile-web-app-status-bar-style" content="black">
		<meta name="apple-mobile-web-app-capable" content="yes">
		<meta name="format-detection" content="telephone=no" />

		<META HTTP-EQUIV="Pragma" CONTENT="no-cache">
		<META HTTP-EQUIV="Cache-Control" CONTENT="no-cache">
		<META HTTP-EQUIV="Expires" CONTENT="0">

		<link rel="stylesheet" type="text/css" href="/Public/Home/css/lib.css" />
		<link rel="stylesheet" type="text/css" href="/Public/Home/css/public.css" />
		<link rel="stylesheet" type="text/css" href="/Public/Home/css/index.css" />
	</head>
	<style>
		#Cocos2dGameContainer{
			width: 100% !important;
			height: 274px !important;
		}
		.top_user{width:100%;color:#bcb6b6;height: 30px;}
		.top_user .headimgurl,.top_user .info,.top_user .fen{display: inline-block;}
		.top_user .headimgurl{width:30px;height: auto}
		.top_user .info{margin-top: 6px;vertical-align: top;font-size: 14px;}
		.top_user .fen{float:right;margin-right:10px;}
		.top_user .btn{
			display: inline-block;
		    background-color: #1fa1dc;
		    width: 60px;
		    height: 24px;
		    line-height: 24px;
		    border-radius: 3px;
		    text-align: center;
		    font-size: 14px;
		    margin-top: 3px;
		    border: 1px solid #1fa1dc;
		    color: #FFF;
		}
		.top_user .logout{background: #f8ac59;border:1px solid #f8ac59;vertical-align: top;margin-top:4px;}
		.top_user .bind{background: #f8ac59;border:1px solid #f8ac59;vertical-align: top;margin-top:4px;}
		.top_user .button-xia{background-color: #1ab394 !important;border:1px solid #1ab394 !important;}
		.top_user .lore{float: right;margin-right:20px;}
		.top_user .register{background-color: #4825e1;border:1px solid #4825e1 !important;}
		.tab-content .wz{padding:0;}
		.tab-box table{padding: 0;font-size:12px;}
	</style>
	<body>
		<div class="tab-content">
			<div class="tab-box wz" data-img='1'>
				<table border="" cellspacing="" cellpadding="">
					<thead>
						<tr>
							<th>期号</th>
							<th>开奖号码</th>
							<th>冠亚</th>
							<th>龙虎</th>
						</tr>
					</thead>
					<tbody>
						<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
								<td><?php echo ($vo["periodnumber"]); ?></td>
								<td class="ft-sz-0">
									<?php
 $number = unserialize($vo['number']); $lh = unserialize($vo['lh']); ?>
									<?php if(is_array($number)): $k = 0; $__LIST__ = $number;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$t): $mod = ($k % 2 );++$k;?><span class="shuzi shuzi_<?php echo ($t); ?>"><?php echo ($t); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
								</td>
								<td class="txt-al-c ft-sz-0" >
									<span class="tm"><?php echo ($vo["tema"]); ?></span>
									<span class="tm cl-<?php echo ($vo["tema_dx"]); ?>"><?php echo ($vo["tema_dx"]); ?></span>
									<span class="tm cl-<?php echo ($vo["tema_ds"]); ?>"><?php echo ($vo["tema_ds"]); ?></span>
									<span class="tm cl-<?php echo ($vo["zx"]); ?>"><?php echo ($vo["zx"]); ?></span>
									<span class="tm"><?php echo ($vo["tema_dw"]); ?></span>
								</td>
								<td>
									<strong class="longhu cl-<?php echo ($lh[0]); ?>"><?php echo ($lh[0]); ?></strong>
									<strong class="longhu cl-<?php echo ($lh[1]); ?>"><?php echo ($lh[1]); ?></strong>
									<strong class="longhu cl-<?php echo ($lh[2]); ?>"><?php echo ($lh[2]); ?></strong>
									<strong class="longhu cl-<?php echo ($lh[3]); ?>"><?php echo ($lh[3]); ?></strong>
									<strong class="longhu cl-<?php echo ($lh[4]); ?>"><?php echo ($lh[4]); ?></strong>
								</td>
							</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					</tbody>
				</table>
			</div>
		</div>
		<style type="text/css">
			.leftRight .nav span{
				width:54px;
				height: 33px;
				display: block;
				color: #FFF;
				font-weight: bold;;
				text-align: center;
				line-height: 33px;
				margin-top:6px;
				border-radius: 5px;
			}
			.headimgurl{text-align: center;}
			.leftRight .nav span.jingcai{background-color: #008dff;}
			.leftRight .nav span.refresh{background-color: #cf3be6;}
			.leftRight .nav span.record{background-color: #4688df;}
			.leftRight .nav span.kaijiang{background-color: #5ac56e;}
			.leftRight .nav span.rule{background-color: #000;}
			.leftRight .nav span.kefu{background-color: #705ac5;}
			.leftRight .nav span.tui{background-color: #31b3a9;}
			.leftRight .nav span.sf{background-color: #1fa1dc;}
			.leftRight .nav span.xf{background-color: #1ab394;}
			.none{display: none}
		</style>
	
	</body>

</html>
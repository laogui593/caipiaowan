...
```sql
CREATE DATABASE laogui CHARACTER SET utf8 COLLATE utf8_general_ci;
...
⚠️ **重要**: 必须授予 `information_schema` 的 SELECT 权限，否则 ThinkPHP 无法正常工作。
...
1. 访问诊断工具: `http://你的域名/数据库权限修复.html`
2. 或运行: `http://你的域名/fix_permissions.php`
3. 按照提示执行 SQL 命令

**详细指南**:
- `数据库权限修复指南.md` - 中文完整指南
- `DATABASE_PERMISSION_FIX.md` - English guide
- `DATABASE_CONFIG.md` - 配置说明
